...
def MNDWI(image):
    return image.expression('(SWIR1-GREEN)/(SWIR1+GREEN)',#
    {
      'GREEN': image.select('green'),
      'SWIR1': image.select('swir1')
    }).rename('MNDWI').copyProperties(image,['satellite','SCENE_CENTER_TIME'])
...
def NDWI(image):
    return image.expression('(NIR-GREEN)/(NIR+GREEN)',#
    {
      'GREEN': image.select('green'),
      'NIR': image.select('nir')
    }).rename('NDWI').copyProperties(image,['satellite','SCENE_CENTER_TIME'])
...
def RGB(image):
        ...
        return new_image.copyProperties(image,['satellite','SCENE_CENTER_TIME'])
#%% Data/Folder 
